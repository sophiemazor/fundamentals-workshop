...
#| out-width: "80%"
#| echo: false
#| fig-cap: "Inspiration for the `magrittr` package name."
...
#| out-width: "40%"
#| echo: false
#| fig-cap: "The `magrittr` package logo."
...
Let's consider the following example of making and eating a cake (thanks to https://twitter.com/dmi3k/status/1191824875842879489?s=09). This would be the traditional way of writing some code (this code block doesn't get evaluated :)
...
bmi.df = df.starwars %>%
  ...
  mutate(height = height / 100, 
       bmi = mass / (height^2)) %>%
  select(name, height, gender, mass, bmi) %>%
  arrange(desc(bmi))
